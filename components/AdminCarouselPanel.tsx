...
import { useCatalog, CarouselImage } from '../context/CatalogContext';
...
const CarouselModal: React.FC<{
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setUploading(true);
    try {
      const payload = { name: name.trim(), image_url: imagePreview };

      if (isEdit && existing?.id) {
        await catalogContext.updatePartnerLogo(existing.id, payload, imageFile);
        toast.success('Carousel image updated!');
      } else {
        await catalogContext.addPartnerLogo(payload, imageFile);
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <div onClick={() => imgRef.current?.click()} className="relative w-full h-36 border-2 border-dashed border-gray-200 rounded-xl flex items-center justify-center cursor-pointer overflow-hidden group hover:border-brand-blue/60 transition-all pointer-events-auto">
                    {imagePreview ? (
                      <>
                        <img src={imagePreview.startsWith('blob:') ? imagePreview : catalogContext.resolveImageUrl(imagePreview)} className="w-full h-full object-contain p-4" alt="preview" />
...
const AdminCarouselPanel: React.FC = () => {
  ...
  const fetchImages = useCallback(() => {
    setLoading(true);
    setImages(catalogContext.partnerLogos);
    setLoading(false);
  }, [catalogContext.partnerLogos]);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await catalogContext.deletePartnerLogo(id);
             ...
             {images.map(img => (
               <div key={img.id} className="grid grid-cols-[100px_1fr_auto] gap-4 px-6 py-4 items-center hover:bg-blue-50/30 transition-colors">
                 <div className="w-20 h-12 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0 border border-black/5 flex items-center justify-center p-2">
                   <img src={catalogContext.resolveImageUrl(img.image_url)} alt={img.name} className="w-full h-full object-contain" />
                 </div>
                 <span className="font-semibold text-gray-900">{img.name}</span>
                 <div className="flex items-center gap-2">
                   <button onClick={() => forceDownload(catalogContext.resolveImageUrl(img.image_url), img.name || 'image.png')} className="p-2 text-gray-400 hover:text-brand-blue bg-white border border-gray-200 shadow-sm rounded-lg transition-colors" title="Download"><Download className="w-4 h-4" /></button>
                   ...
                   <button onClick={() => handleDelete(img.id)} className="p-2 text-gray-400 hover:text-red-500 bg-white border border-gray-200 shadow-sm rounded-lg transition-colors" title="Delete"><Trash2 className="w-4 h-4" /></button>
                 </div>
               </div>
             ))}
          </div>
